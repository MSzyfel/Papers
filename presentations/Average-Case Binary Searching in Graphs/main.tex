...
\date{\today, University of Gdańsk}
...
\begin{itemize}
    ...
    \pause
    \item Let $\beta=\frac{1-\delta}{2}$. We have that the contribution of the decision tree $D_{\mathcal{T}}$ is bounded by:
    \pause
            \begin{align*}
...
\begin{frame}{Sketch of the proof}
    \begin{lemma}\label{up_trees}
            ...
            \end{lemma}
    \begin{itemize}
    \item Let $D$ be the resulting decision tree. We have:
    \begin{align*}
            c_T\br{D} &\leq 
...
\begin{frame}{
    What's next?}
\begin{itemize}
    \item An FPTAS for graphs with bounded number of inner vertices based on a dynamic program for scheduling with rejection.
    \pause
    \item We are working on the case when the cost incurred by a query depends on the target vertex. This cannot be approximated to within $\br{2-\epsilon}$ even when the input is a star via a reduction from $1|prec| \sum w_iC_i$.
    \pause
    \item For trees, when the cost is monotone non-decreasing with respect to the target i. e. if $u \in P_{v,x}$ then $c\br{u,x} \leq c\br{v,x}$, we have found a $2$-approximation.
    \pause
    \end{itemize}
\end{frame}
\begin{frame}{Summary}
\begin{itemize}
    \item The key idea behind our lower bounds is to decompose the optimal decision tree into series of separators
    \item This connection immediately yields approximation algorithms for the Graph Search Problem based on approximation algorithms for the Weighted $\alpha$–Separator Problem and the Min–Ratio Vertex Cut Problem.
    \item The approximation ratio is $\br{4+\epsilon}$, and $O\br{\sqrt{\log n}}$ for general graphs.
\end{itemize}
\end{frame}
