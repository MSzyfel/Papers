...
\title{Approximating the Average-Case Graph Search Problem with Non-Uniform Costs}
\author{Michał Szyfelbein}
\institute{Gdańsk University of Technology, Poland}
\date{March 5, WALCOM 2026}
...
\textbf{Binary Search} – a classical strategy used to efficiently locate a hidden \textbf{target} element $x$ in a linearly ordered set using $O\br{\log n}$ comparison operations.
...
\begin{frame}\frametitle{Additional parameters}
    ...
    \item 
    Additionally, to each vertex $v$ we also assign an arbitrary \textbf{weight} $w\br{v}$, which denotes the importance of $v$.
    % \pause
...
%         \item Parallel assembly of multi–part products.
%         \item VLSI routing and layer assignment.
%         \item Fast implementation of distance and shortest path queries.
%     \end{itemize}
% \end{frame}
% \begin{frame}{Why do we care?}
% \textbf{Why our setup?:}
% \begin{enumerate}
%     \item \textbf{Average case} – it is natural to assume that the search strategies we design are intended to be used repeatedly. \pause
%     \item \textbf{Weight function} – some vertices may serve 
% as targets more frequently than the others. \pause
%     \item \textbf{Query costs} – performing a query may require 
% significant resources, such as time or money. \pause
...
% Notation: \textbf{search space $||$ query model $||$ objective function:}
%     \pause
% \begin{itemize}
%     \item $T||V||\sum C_i$:
%     \begin{itemize}
%         \item $\br{1+\epsilon}$ in
% $O\br{\br{1/\epsilon}^{2/\log_23}\cdot n^{1+4/\log_23}\cdot \log^2\br{n/\epsilon}}$ time.
%     \item Every optimal decision tree has height at most $O\br{\log w\br{T}}$.
%     \item Weighted centroid decision tree is 2-approximate.
%     \end{itemize}
%     \pause
%     \item $T||V|| C_{max}$:
%     \begin{itemize}
%         \item Solvable in $O\br{n}$ time, $\fl{\log n}+1$ queries always suffice.
%     \end{itemize}
%     \pause
%     \item $G||V|| C_{max}$
%     \begin{itemize}
%         \item NP-hard, best known approximation is $O\br{\log^{3/2} n}$.
%     \end{itemize}
%     \pause
%     \item $T||V, c|| C_{max}$
%     \begin{itemize}
%         \item NP-hard, best known approximation is $O\br{\sqrt{\log n}}$.
%     \end{itemize}
%     \pause
%     \item $G||V, c|| C_{max}$
%     \begin{itemize}
%         \item Reducible to the uniform cost version.
%     \end{itemize}
% \end{itemize}
% \end{frame}
...
% \footnotesize
% \textbf{The Graph Search Problem appears in the literature under many different names:}
% \begin{itemize}
...
\begin{frame}
\frametitle{Cost of searching}
\input{figures/sample_decision_trees_for_graph_highlighted.tex}
\end{frame}

\begin{frame}
\frametitle{Our results}
This problem is \textbf{NP–hard} already for trees. We give the following \textbf{approximation} algorithms:
\begin{itemize}
    \item $\br{4+\epsilon}$–approximation for \textbf{trees}.
    \pause
    \item $O\br{f_n}$-approximation algorithm, where $f_n$ is the approximation ratio of any polynomial time algorithm for the Min–Ratio Vertex Cut Problem. In particular, this yields:
    \begin{itemize}
        \pause
        \item $O\br{\sqrt{\log n}}$–approximation for \textbf{general graphs}.
        \item $O\br{\spr{V\br{H}}^2}$-approximation for graphs excluding $H$ as a minor which yields constant factor approximation for many important graph classes, such as planar graphs, graphs of bounded treewidth, etc.
    \end{itemize}
\end{itemize}
\end{frame}
...
\textbf{Bad news}: The Weighted $\alpha$–separator Problem is \textbf{NP–hard} even for stars.
...
%     \end{lemma}
%     \begin{lemma}
%     Let $T$ be a tree, $\mathcal{T}$ - any subtree of $T$ and $0<\beta<1$ - some constant. We have:
%     \begin{align*}
...
\begin{lemma}\label{lb_opt}
    $$
    2\cdot\OPT\br{G}= 2\cdot\sum_{k=0}^{w\br{G}-1}c\br{S_{k}^*} \geq \sum_{k=0}^{w\br{G}}c\br{S_{\fl{k/2}}^*}.
    $$
\end{lemma}
    \pause
...
\begin{itemize}
    ...
    \item Let $S_{\mathcal{T}}^*=S_{\fl{w\br{\mathcal{T}}/2}}^*\cap\mathcal{T}$. We have $c\br{S_{\mathcal{T}}}\leq c\br{S_{\mathcal{T}}^*}$.
    \item We have that the contribution of the decision tree $D_{\mathcal{T}}$ is bounded by:
    \pause
            \begin{align*}
                w\br{\mathcal{T}}\cdot c\br{S_{\mathcal{T}}}
                &\leq w\br{\mathcal{T}}\cdot c\br{S_{\mathcal{T}}^*} 
                \\&\leq 
                \frac{2}{1-\delta}\cdot \sum_{k=\frac{1+\delta}{2}\cdot w\br{\mathcal{T}}+1}^{w\br{\mathcal{T}}}c\br{S_{\fl{k/2}}^*\cap \mathcal{T}}.
            \end{align*}
    \end{itemize} 
\end{frame}
...
\begin{frame}{Sketch of the proof}
    ...
    \end{align*}
    \pause
    \item As $1/\delta=\frac{4+\epsilon}{\epsilon}=1+4/\epsilon$, the running time is $O\br{n^4/\epsilon^2}$.
    \end{itemize}
\end{frame}
...
We use the following result of \cite{Improvedapproximationalgorithmsvertexseparators}:
\begin{theorem}\label{approxmrvc}
    Given a graph $G=\br{V\br{G}, E\br{G}}$, the cost function $c\colon V\to\mathbb{N}$ and the weight function $w\colon V\to \mathbb{N}$, there exists a
...
\begin{frame}{Sketch of the proof}
    ...
    \begin{itemize}
        ...
        \item Let $S_{\mathcal{G}}^*=S_{\fl{w\br{\mathcal{G}}/2}}^*\cap \mathcal{G}$.
    \end{itemize}
        \pause
        \begin{lemma}\label{lambda_lemma}
            Let $\mathcal{H}=\mathcal{G}-S_{\mathcal{G}}^*$. Then, we can partition $\mathcal{H}$ into two sets, $\mathcal{A}$ and $\mathcal{B}$ such that for $A=\bigcup_{H\in\mathcal{A}}V\br{H}$ and $B=\bigcup_{H\in\mathcal{B}}V\br{H}$, we have:
            $$w\br{A\cup S_{\mathcal{G}}^*}\cdot w\br{B\cup S_{\mathcal{G}}^*}\geq w\br{\mathcal{G}}^2/11.$$
        \end{lemma}
        The partition $\br{A,S_{\mathcal{G}}^*,B}$ in the above lemma is a vertex cut of $\mathcal{G}$, so we have:
                $$\alpha_{c,w}\br{\mathcal{G}}\leq \frac{c\br{S_{\mathcal{G}}^*}}{w\br{A\cup S_{\mathcal{G}}^*}\cdot w\br{B\cup S_{\mathcal{G}}^*}}\leq \frac{11 \cdot c\br{S_{\mathcal{G}}^*}}{w\br{\mathcal{G}}^2}.
                $$
    \end{frame}
...
\begin{frame}{Sketch of the proof}
    \begin{itemize}
        \item Let $\br{A_{\mathcal{G}},S_{\mathcal{G}}, B_{\mathcal{G}}}$, be the partition returned by the algorithm:
        $$
        \alpha_{c,w}\br{A_{\mathcal{G}},S_{\mathcal{G}}, B_{\mathcal{G}}}=\frac{c\br{S_{\mathcal{G}}}}{w\br{A_{\mathcal{G}}\cup S_{\mathcal{G}}}\cdot w\br{B_{\mathcal{G}}\cup S_{\mathcal{G}}}}\leq f_n \cdot \frac{11\cdot c\br{S_{\mathcal{G}}^*}} {w\br{\mathcal{G}}^2}.
        $$
        % \pause
        \item Let $\beta=w\br{B_{\mathcal{G}}\cup S_{\mathcal{G}}}/w\br{\mathcal{G}}$, so that $\br{1-\beta}\cdot w\br{\mathcal{G}} = w\br{A_\mathcal{G}}$.
        \item Assume w. l. o. s. that $w\br{A_{\mathcal{G}}}\geq w\br{B_{\mathcal{G}}}$. 
        \pause
        \item The contribution of the decision tree $D_{\mathcal{G}}$ is bounded by:
        \begin{align*}
        w\br{\mathcal{G}}\cdot c\br{S_{\mathcal{G}}} &\leq 11 \cdot f_n\cdot \frac{w\br{A_{\mathcal{G}}\cup S_{\mathcal{G}}}\cdot w\br{B_{\mathcal{G}}\cup S_{\mathcal{G}}}}{w\br{\mathcal{G}}}\cdot c\br{S_{\mathcal{G}}^*}\\
        & \leq 
        11 \cdot f_n\cdot w\br{B_{\mathcal{G}}\cup S_{\mathcal{G}}}\cdot c\br{S_{\mathcal{G}}^*}
        \\ & \leq 
...
\begin{frame}{Sketch of the proof}
    Let $D$ be the decision tree returned by the procedure. We have:
        \begin{align*}
            ...
            &\leq 11 \cdot f_n\cdot \sum_{\mathcal{G}}\sum_{k=w\br{A_{\mathcal{G}}}+1}^{w\br{\mathcal{G}}}c\br{S_{\fl{k/2}}^*\cap \mathcal{G}}
            \\&\leq 
            11 \cdot f_n\cdot\sum_{k=0}^{w\br{G}}c\br{S_{\fl{k/2}}^*} \\
            &\leq 22 \cdot f_n\cdot \OPT\br{G}
        \end{align*}
\end{frame}
% \begin{frame}{
%     What's next?}
% \begin{itemize}
%     \item The results for the \textbf{average case} variant of the Graph Search Problem far \textbf{outperform} those for the \textbf{worst case} variant:
%     \begin{itemize}
%     \item For trees: $\br{4+\epsilon}$ vs. $O\br{\sqrt{\log n}}$,
%     \item For general graphs: $O\br{\sqrt{\log n}}$ vs  $O\br{\log^{3/2}n}$.
%     \end{itemize}
%     \pause
%     \item \textbf{Greedy} algorithms do not work well in the \textbf{worst case} setup. However we think there is a different way of connecting searching and separating which may yield a constant factor approximation for trees.
%     \pause
%     \item For general graphs we are working on an $O\br{\log n}$ approximation.
%     \end{itemize}
% \end{frame}
\begin{frame}{Summary}
\begin{itemize}
    \item The key idea behind our lower bounds is to decompose the optimal decision tree (and its cost) into a series of separators.
    \pause
    \item This immediately yields approximation algorithms for the Graph Search Problem:  $\br{4+\epsilon}$ for trees and $O\br{\sqrt{\log n}}$ for general graphs.
    \pause
    \item Currently, we are working on an FPTAS parametrized by the number of non-leaf vertices of the graph as well as a more general model in which the cost of a query depends on the target vertex.
\end{itemize}
\end{frame}
