...
\subsection{A warm up: $O\br{\log n/\log\log n}$-approximation algorithm  for $T||V,c||\COST_{max}$}
\label{sec:log_n_log_log_n_approx}
...
To construct a decision tree we will use the the procedure from the lemma below:
\begin{lemma}
    There exists an $O\br{2^nn}$ algorithm for $T||V,c||C_{max}$
    \begin{proof}
        The algorithm is a general version of the dynamic programming procedure for paths. We have that:
        $$
        \OPT\br{T,c} = \min_{v\in V\br{T}}\brc{c\br{v}+\max_{H\in T-v}\brc{\OPT\br{H,c}}}.
        $$

        Notice, that there are at most $O\br{2^n}$ different subtrees of $T$ to be checked. Additionally, for each $v\in V\br{T}$, there are at most $\deg_T\br{v}$ possible responses to check in the inner $\max$ function. Therefore, for each subproblem, there are at most
        $
...
We will denote the above procedure as $\FExact$.
...
If otherwise, to build a solution we will firstly define a set $\mathcal{X}\subseteq V\br{\mathcal{T}}$ which will be of size at most $k$. We build $\mathcal{X}$ iteratively. Starting with an empty set we pick the centroid $x_1$ of $T$ which we add to $\mathcal{X}$. Then we take the forest $\mathcal{F}=T-x$, find the largest $H\in \mathcal{F}$, pick its centroid $x_2$ and append it to $\mathcal{X}$. We continue this in $\br{\mathcal{F}-H} \cup \br{H-x_2}$ until $\spr{\mathcal{X}}=k$.
\begin{lemma}\label{lemma:componentSize}
    For every $H\in \mathcal{T}-\mathcal{X}$ we have that $n\br{H}\leq n\br{\mathcal{T}}/\log\br{n}$.
    \begin{proof}
        We prove by induction on $t$ that deleting first $2^t$ centroids from $T$ each connected components $H_t$ has size at most $n\br{H_t}\leq n\br{\mathcal{T}}/2^{t-1}$. For the case when $t=0$ we have that after 1 iteration every $H_1$ has size at most $n\br{\mathcal{T}}/2\leq 2n$ so the base of induction is complete.

        Fix $t>0$ and assume by the induction hypothesis that after $2^{t-1}$ iterations all components of $\mathcal{T}-\mathcal{X}$ have size at most $n\br{\mathcal{T}}/2^{t-2}$. Let $p$ denote the number of such components that have size at least $n\br{\mathcal{T}}/2^{t-1}$ so that $p\leq 2^{t-1}$. By the property of centroids, after deleting one centroid from each of these components, each resulting component has size at most $n\br{\mathcal{T}}/2^{t-1}$. Therefore, after deleting $n\br{\mathcal{T}}/2^{t-1}+p\leq n\br{\mathcal{T}}/2^{t}$ centroids, we have that all resulting components have size at most $n\br{\mathcal{T}}/2^t$. This completes the induction step and the claim follows by the choice of $k$.
    \end{proof}
\end{lemma}
...
\begin{observation}\label{observation:CostDZinT}
    $\COST_{\mathcal{T}_{\mathcal{Z}}}\br{D_{\mathcal{Z}}}=\COST_{\mathcal{T}}\br{D_{\mathcal{Z}}}$.
\end{observation}
...
\begin{lemma}
    ...
    \begin{proof}
        ...
        \begin{align*}
        \APP_{D_{\mathcal{T}}}\br{\mathcal{T}}&=\frac{\COST_{\mathcal{T}}\br{D_{\mathcal{T}}}}{\OPT\br{\mathcal{T}}}\\
        &\leq \frac{\COST_{\mathcal{T}}\br{D_{\mathcal{Z}}}+\max_{H\in \mathcal{T}-\mathcal{Z}}\brc{C_{\mathcal{T}}\br{D_H}}}{\OPT\br{\mathcal{T}}}\\
        &\leq 
        \frac{\COST_{\mathcal{T}_{\mathcal{Z}}}\br{D_{\mathcal{Z}}}}{\OPT\br{\mathcal{T}_{\mathcal{Z}}}}+\max_{H\in \mathcal{T}-\mathcal{Z}}\brc{\frac{C_{\mathcal{T}}\br{D_H}}{\OPT\br{H}}}\\
