...
\subsection{QPTAS for $T||V,c||\COST_{max}$}\label{qptasSection}
...
\begin{observation}\label{basicBoundsOnCost}
    Let $T$ be a tree such that $\spr{V\br{T}}> 1$ and $c:V\to \mathbb{R}^+$ be a normalized cost function. Then, $1\leq \OPT(T) \leq \fl{\log n}+1$.
    \begin{proof}
...
\begin{lemma}\label{rounded_dt_lemma}
    ...
    $$
    \begin{proof}
        Let $D^*$ be an optimal strategy for $\br{T,c}$. By definition, we have that for every vertex $v\in V\br{T}$, $c'\br{v}\leq \br{1+\frac{1}{p}}\cdot c\br{v}+\frac{n}{p}n$ and therefore:
        \begin{align*}
            \OPT\br{T, c'}&\leq \COST_{T}\br{D^*,c'}=\max_{v\in V\br{T}}\brc{\sum_{q\in Q_{D^*}\br{T,v}}c'\br{q}}
            \\&
...
\begin{lemma}\label{aligned_dts_lemma}
    There exists a decision tree $D$ for $\br{T,c'}$, such that $\COST_{T}\br{D, c'}\leq \br{1+\frac{3}{p}}\cdot\OPT\br{T, c'}$ and for every vertex $v\in V\br{T}$ we have:
    \begin{enumerate}
    ...
    \end{enumerate}
    \begin{proof}
        Let $D^*$ by any optimal decision tree for $\br{T,c'}$. For any $v\in V\br{T}$, let $t_v^*$ be the start of query to $v$ in $D^*$ and let $t_v'=\br{1+\frac{2}{p}}t_v^*$, yielding a new decision tree $D'$. Since in $D'$, the ordering of vertices is exactly the same as in $D^*$, for any two consecutive queries $v,u$ in $D'$ we have:
        $$
...
We will call a decision tree fulfilling above conditions \textit{aligned}. In subsequent considerations, we will limit ourselves to finding only aligned decision trees, whose properties will allow us to devise an efficient dynamic programming procedure finding an optimal, aligned decision tree. 

\subsubsection{Up and down responses, heavy module contraction and aligned cost}
Since our decision tree is rooted, we can reasonably talk about up and down responses to a query. An \textit{up} response to a query to $v$ in $T$ occurs when the connected component $\in T-v$, which is the reply, happens to contain $r\br{T}$. If this is not the case, then such response is called a \textit{down} response. As it will turn out, a repeating occurrence of light queries with down responses will become problematic for our algorithm. To account for this issue we will use the following notions:

We define a new measure of cost for aligned decision trees called the \textit{aligned cost}. Let $D$ be any aligned strategy for $\br{T,c'}$. For any vertex $v\in V\br{T}$ and query $q\in Q_{D}\br{T,v}$ the contribution $\kappa_{T,c,k}\br{q,v}$ of $u$ is defined as:
$$
...
$$
\COST'_{T}\br{D,c',k}=\max_{v\in V\br{T}}\brc{\sum_{q\in Q_D\br{T,v}}\kappa_{T,c', k}\br{q,v}}.
$$

Let $\OPT'\br{T,c',k}$ denote the optimal aligned cost among all aligned decision trees for $\br{T,c',k}$.
Notice that in the above cost, we lose up to $k$ units of time per each light query with a down response. Since of course, the amount of such queries may be of order $O\br{n}$, the difference between $\COST'_{T}\br{D,c',k}$ and $\COST_{T}\br{D,c',k}$ may grow almost arbitrarily large. However, we will make sure that this does not happen to often, which will give us the desired bound on the cost of the solution. We have the following simple observation:
...
\begin{proposition}\label{DPTimelinesCostsProposition}
     Let $T$ be a tree, $c'$ an aligned cost function, $p\in \mathbb{N}$, $k$ the box size and $d\in \mathbb{N}$~be the depth. There exists a \FDPTimelinesCosts procedure, which (if it exists) calculates an aligned decision tree $D$ for $\br{T,c'}$ of cost at most $\COST'_{T}\br{D,c',k}\leq kd$, running in $\br{pn}^{O\br{d}}$ time.
\end{proposition}
\begin{proposition}\label{MergeDTsProposition}
    Let $T$ be a tree, $c'$ be an aligned cost function, $p\in \mathbb{N}$, $k\in \mathbb{R}_{>0}$ be the box size, $D_A$ be a decision tree for $T$ and $F_C$ be forest of decision trees for $T$ with all heavy modules contracted. There exists a polynomial time \FMergeDTs procedure which returns a decision tree of cost at most:
        $$
            \COST_{T}\br{D,c',k}\leq\COST'_{T}\br{D_A,c',k}+2pk\cdot \COST_{T}\br{F_C,1}.
        $$
\end{proposition}
...
\begin{align*}
    \COST_{T}\br{D,c'}&\leq \OPT'\br{T,c',k'}+2pk'\cdot \br{\fl{\log n}+1}
...
\begin{proof}
    ...
    \begin{enumerate}
        \item $r$ is heavy or $r$ is light and the response is up. In this case the cost of query to $r$ is incorporated into $\COST'_{T}\br{D_A,c',k}$.
        \item $r$ is light and the response is down. Let  $r_1$ and $r_2$ be two consecutive light queries in $Q_D\br{T,x}$ with a down response, and let $T''\in T-\brc{r_1, r_2}$, such that $x\in T''$. We will show that $\COST_{T}\br{F_C|T,1}\leq \COST_{T}\br{F_C,1}-1$, so the additional cost of such queries is at most $2pk\cdot \COST_{T}\br{F_C,1}$.
        \begin{enumerate}
            \item $F_C$ is connected. In such case $r_1=r\br{F_C}$ so after query to $r_1$, by definition of the decision tree $\COST_{T}\br{F_C|T',1}\leq \COST_{T}\br{F_C,1}-1$.
            
            \item $F_C$ is disconnected. Let $T'$ be any down response to query to $r$. If $r_1=r\br{F_C|T'}$, then $\COST_{T}\br{F_C|T',1}\leq \COST_{T}\br{F_C,1} - 1$. Otherwise, we argue that $F_C|T'$ is connected. Assume contrary. Firstly, we observe that $F_{C}|T_{r_1}$ is connected. This is because, $r_1$ is light, so the tree $T_{r_1,C}$ denoting $T_{r_1}$ with all heavy modules contracted is connected. By assumption, there at least two connected components $D_1,D_2,\dots,D_j$ of $F_C|T'$, with roots $d_1,d_2,\dots, d_j$ respectively. Since every other query $q\in V\br{F_C|T'}$ is a descendant of some $d_1,\dots,d_j$, the only query which could be the parent of $d_1,\dots, d_j$ in $F_C|T'$ is $r_1$, which is a contradiction. Therefore, the next query $r_2$ of $D$ in $T'$ is $r\br{F_C|T'}$. We get that by definition of the decision tree $\COST_{T}\br{F_C|T''}\leq \COST_{T}\br{F_C,1}-1$.
        \end{enumerate} 
    \end{enumerate}
\end{proof}
\end{lemma}
...
\begin{lemma}
    The running time of the \FDPTimelinesCosts procedure is bounded by $\br{pn}^{O\br{d}}$.
    \begin{proof}
    The number of subtrees $T_{v,i}$ is $O\br{n}$. Each box in the boxline can have one of at most $O\br{n}$ values of load and at most 2 values of the boolean flag. Since each boxline is of length $d$, number of different boxlines for each $T_{v,i}$ is bounded by $\br{pn}^{O\br{d}}$. Additionally, the running time of each subproblem is bounded by $\br{pn}^{O\br{d}}$, since there at most this many bipartitions of available load and boolean flags among boxes. Therefore, the overall running time is bounded by $\br{pn}^{O\br{d}}$.
    \end{proof}
\end{lemma}
