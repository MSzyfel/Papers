...
\begin{lemma}
    ...
    $$
    \begin{proof}
        By definition, for every $v\in V\br{T}$, we have $w'\br{v}\leq \frac{w\br{v}}{K}+1$ and therefore $K\cdot w'\br{v}\leq w\br{v}+K$. Let $D^*$ be the optimal solution for the $\br{T,w}$ instance. We have:
        \begin{align*}
...
\end{lemma}
    ...
    Hence, the running time of the procedure is bounded by: 
    $$O\br{n\cdot 2^{2h}\cdot h}=O\br{n\cdot w'\br{T}^{2\cdot\log_{3/2}\br{2}}\cdot \log w'\br{T}}=O\br{n\cdot \br{n/\epsilon}^{4\cdot\log_{3/2}\br{2}}\cdot\log\br{n/\epsilon}}$$ and the claim follows.
\end{proof}
\end{theorem}

It should be noted that despite the fact that the above algorithm is an FPTAS, the exponent of the polynomial of $n$ is $4\log_{3/2}\br{2}+1\approx 7.838$ (ignoring polylogarithmic factors). This makes the algorithm impractical for most inputs and yields the greedy algorithm querying weighted centroid a preferential choice for most applications. A similar issue concerns the PTAS from the previous section. For $\epsilon=1$ the running time of the latter is $O\br{n^{5}}$ which is much worse then the $O\br{n\log n}$ time achieved by the greedy algorithm. The PTAS is therefore useful for $\epsilon \in (0.413, 1)$ when it outperforms the FPTAS. 
