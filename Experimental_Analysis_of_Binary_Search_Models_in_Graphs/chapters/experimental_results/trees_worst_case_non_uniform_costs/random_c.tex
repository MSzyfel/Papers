\FloatBarrier\subsection{Random Trees with Uniform Cost Distribution ($T_R || V, c \sim \mathcal{U} || \COST$)}
\label{sec:random_c_uniform}

\subsubsection{DP Exact Algorithm}

\begin{table}[ht]
\centering
\tiny
\caption{Random trees with uniform cost distribution - DP exact algorithm}
\label{tab:random_c_uniform_dp}
\begin{tabular}{|c|c|c|c|c|}
\hline
$n$ & Avg Time (s) & Max Time (s) & Avg Cost & Max Cost \\
\hline
\csvreader[head to column names, late after line=\\]{data/dp_tree/dp_tree_basic_cu_analysis.csv}{Size=\Size,AvgCPUTime_s=\AvgTime,MaxCPUTime_s=\MaxTime,AvgCost=\AvgCost,MaxCost=\MaxCost,InstanceCount=\Instances}{%
\Size & \AvgTime & \MaxTime & \AvgCost & \MaxCost}
\hline
\end{tabular}
\end{table}

\begin{figure}[ht]
\centering
\begin{tikzpicture}
\begin{axis}[
    xlabel={Tree size ($n$)},
    ylabel={Cost},
    width=\textwidth,
    height=7cm,
    grid=major,
    legend pos=north west,
    mark size=2pt
]
\addplot[blue, mark=*, thick] table[x=Size, y=AvgCost, col sep=comma] {data/dp_tree/dp_tree_basic_cu_analysis.csv};
\addplot[red, mark=square*, thick] table[x=Size, y=MaxCost, col sep=comma] {data/dp_tree/dp_tree_basic_cu_analysis.csv};
\legend{Avg Cost, Max Cost}
\end{axis}
\end{tikzpicture}
\caption{Random trees with uniform costs - DP exact algorithm: cost}
\label{fig:random_c_uniform_dp_cost}
\end{figure}

\begin{figure}[ht]
\centering
\begin{tikzpicture}
\begin{axis}[
    xlabel={Tree size ($n$)},
    ylabel={Runtime (s)},
    width=\textwidth,
    height=7cm,
    grid=major,
    legend pos=north west,
    mark size=2pt
]
\addplot[blue, mark=*, thick] table[x=Size, y=AvgCPUTime_s, col sep=comma] {data/dp_tree/dp_tree_basic_cu_analysis.csv};
\addplot[red, mark=square*, thick] table[x=Size, y=MaxCPUTime_s, col sep=comma] {data/dp_tree/dp_tree_basic_cu_analysis.csv};
\legend{Avg Time, Max Time}
\end{axis}
\end{tikzpicture}
\caption{Random trees with uniform costs - DP exact algorithm: runtime}
\label{fig:random_c_uniform_dp_time}
\end{figure}

\FloatBarrier

\subsubsection{Cicalese et al. $O(\log n / \log \log n)$-Approximation Algorithm}

\begin{table}[ht]
\centering
\tiny
\caption{Random trees with uniform cost distribution - Cicalese algorithm (every 2nd instance)}
\label{tab:random_c_uniform_cic}
\begin{tabular}{|c|c|c|c|c|c|c|}
\hline
$n$ & Avg Time (s) & Max Time (s) & Avg Cost & Max Cost & Avg Ratio & Max Ratio \\
\hline
\csvreader[head to column names, late after line=\\]{data/cic_insp/cic_insp_basic_cu_comparison.csv}{Size=\Size,AvgCPUTime_s=\AvgTime,MaxCPUTime_s=\MaxTime,AvgCost=\AvgCost,MaxCost=\MaxCost,AvgRatio=\AvgRatio,MaxRatio=\MaxRatio}{%
\Size & \AvgTime & \MaxTime & \AvgCost & \MaxCost & \AvgRatio & \MaxRatio}
\hline
\end{tabular}
\end{table}

\begin{figure}[ht]
\centering
\begin{tikzpicture}
\begin{axis}[
    xlabel={Tree size ($n$)},
    ylabel={Approximation ratio},
    width=\textwidth,
    height=7cm,
    grid=major,
    legend pos=north west,
    mark size=2pt
]
\addplot[blue, mark=*, thick] table[x=Size, y=AvgRatio, col sep=comma] {data/cic_insp/cic_insp_basic_cu_comparison.csv};
\addplot[red, mark=square*, thick] table[x=Size, y=MaxRatio, col sep=comma] {data/cic_insp/cic_insp_basic_cu_comparison.csv};
\legend{Avg Ratio, Max Ratio}
\end{axis}
\end{tikzpicture}
\caption{Random trees with uniform costs - Cicalese algorithm: approximation quality}
\label{fig:random_c_uniform_cic}
\end{figure}

\begin{figure}[ht]
\centering
\begin{tikzpicture}
\begin{axis}[
    xlabel={Tree size ($n$)},
    ylabel={Runtime (s)},
    width=\textwidth,
    height=7cm,
    grid=major,
    legend pos=north west,
    mark size=2pt
]
\addplot[blue, mark=*, thick] table[x=Size, y=AvgCPUTime_s, col sep=comma] {data/cic_insp/cic_insp_basic_cu_comparison.csv};
\addplot[red, mark=square*, thick] table[x=Size, y=MaxCPUTime_s, col sep=comma] {data/cic_insp/cic_insp_basic_cu_comparison.csv};
\legend{Avg Time, Max Time}
\end{axis}
\end{tikzpicture}
\caption{Random trees with uniform costs - Cicalese algorithm: runtime}
\label{fig:random_c_uniform_cic_time}
\end{figure}

\FloatBarrier

\subsubsection{Dereniowski $O(\sqrt{\log n})$-Approximation Algorithm}

\begin{table}[ht]
\centering
\tiny
\caption{Random trees with uniform cost distribution - Dereniowski algorithm (every 2nd instance)}
\label{tab:random_c_uniform_der}
\begin{tabular}{|c|c|c|c|c|c|c|}
\hline
$n$ & Avg Time (s) & Max Time (s) & Avg Cost & Max Cost & Avg Ratio & Max Ratio \\
\hline
\csvreader[head to column names, late after line=\\]{data/der_insp/der_insp_basic_cu_comparison.csv}{Size=\Size,AvgCPUTime_s=\AvgTime,MaxCPUTime_s=\MaxTime,AvgCost=\AvgCost,MaxCost=\MaxCost,AvgRatio=\AvgRatio,MaxRatio=\MaxRatio}{%
\Size & \AvgTime & \MaxTime & \AvgCost & \MaxCost & \AvgRatio & \MaxRatio}
\hline
\end{tabular}
\end{table}

\begin{figure}[ht]
\centering
\begin{tikzpicture}
\begin{axis}[
    xlabel={Tree size ($n$)},
    ylabel={Approximation ratio},
    width=\textwidth,
    height=7cm,
    grid=major,
    legend pos=north west,
    mark size=2pt
]
\addplot[blue, mark=*, thick] table[x=Size, y=AvgRatio, col sep=comma] {data/der_insp/der_insp_basic_cu_comparison.csv};
\addplot[red, mark=square*, thick] table[x=Size, y=MaxRatio, col sep=comma] {data/der_insp/der_insp_basic_cu_comparison.csv};
\legend{Avg Ratio, Max Ratio}
\end{axis}
\end{tikzpicture}
\caption{Random trees with uniform costs - Dereniowski algorithm: approximation quality}
\label{fig:random_c_uniform_der}
\end{figure}

\begin{figure}[ht]
\centering
\begin{tikzpicture}
\begin{axis}[
    xlabel={Tree size ($n$)},
    ylabel={Runtime (s)},
    width=\textwidth,
    height=7cm,
    grid=major,
    legend pos=north west,
    mark size=2pt
]
\addplot[blue, mark=*, thick] table[x=Size, y=AvgCPUTime_s, col sep=comma] {data/der_insp/der_insp_basic_cu_comparison.csv};
\addplot[red, mark=square*, thick] table[x=Size, y=MaxCPUTime_s, col sep=comma] {data/der_insp/der_insp_basic_cu_comparison.csv};
\legend{Avg Time, Max Time}
\end{axis}
\end{tikzpicture}
\caption{Random trees with uniform costs - Dereniowski algorithm: runtime}
\label{fig:random_c_uniform_der_time}
\end{figure}

\FloatBarrier

\subsubsection{K-up $O(\log \log n)$-Approximation Algorithm}

\begin{table}[ht]
\centering
\tiny
\caption{Random trees with uniform cost distribution - K-up algorithm (every 2nd instance)}
\label{tab:random_c_uniform_kup}
\begin{tabular}{|c|c|c|c|c|c|c|}
\hline
$n$ & Avg Time (s) & Max Time (s) & Avg Cost & Max Cost & Avg Ratio & Max Ratio \\
\hline
\csvreader[head to column names, late after line=\\]{data/k_up/k_up_basic_cu_comparison.csv}{Size=\Size,AvgCPUTime_s=\AvgTime,MaxCPUTime_s=\MaxTime,AvgCost=\AvgCost,MaxCost=\MaxCost,AvgRatio=\AvgRatio,MaxRatio=\MaxRatio}{%
\Size & \AvgTime & \MaxTime & \AvgCost & \MaxCost & \AvgRatio & \MaxRatio}
\hline
\end{tabular}
\end{table}

\begin{figure}[ht]
\centering
\begin{tikzpicture}
\begin{axis}[
    xlabel={Tree size ($n$)},
    ylabel={Approximation ratio},
    width=\textwidth,
    height=7cm,
    grid=major,
    legend pos=north west,
    mark size=2pt
]
\addplot[blue, mark=*, thick] table[x=Size, y=AvgRatio, col sep=comma] {data/k_up/k_up_basic_cu_comparison.csv};
\addplot[red, mark=square*, thick] table[x=Size, y=MaxRatio, col sep=comma] {data/k_up/k_up_basic_cu_comparison.csv};
\legend{Avg Ratio, Max Ratio}
\end{axis}
\end{tikzpicture}
\caption{Random trees with uniform costs - K-up algorithm: approximation quality}
\label{fig:random_c_uniform_kup}
\end{figure}

\begin{figure}[ht]
\centering
\begin{tikzpicture}
\begin{axis}[
    xlabel={Tree size ($n$)},
    ylabel={Runtime (s)},
    width=\textwidth,
    height=7cm,
    grid=major,
    legend pos=north west,
    mark size=2pt
]
\addplot[blue, mark=*, thick] table[x=Size, y=AvgCPUTime_s, col sep=comma] {data/k_up/k_up_basic_cu_comparison.csv};
\addplot[red, mark=square*, thick] table[x=Size, y=MaxCPUTime_s, col sep=comma] {data/k_up/k_up_basic_cu_comparison.csv};
\legend{Avg Time, Max Time}
\end{axis}
\end{tikzpicture}
\caption{Random trees with uniform costs - K-up algorithm: runtime}
\label{fig:random_c_uniform_kup_time}
\end{figure}

\FloatBarrier\subsection{Random Trees with Binomial Cost Distribution ($T_R || V, c \sim \mathcal{B} || \COST$)}
\label{sec:random_c_binomial}

\subsubsection{DP Exact Algorithm}

\begin{table}[ht]
\centering
\tiny
\caption{Random trees with binomial cost distribution - DP exact algorithm}
\label{tab:random_c_binomial_dp}
\begin{tabular}{|c|c|c|c|c|}
\hline
$n$ & Avg Time (s) & Max Time (s) & Avg Cost & Max Cost \\
\hline
\csvreader[head to column names, late after line=\\]{data/dp_tree/dp_tree_basic_cb_analysis.csv}{Size=\Size,AvgCPUTime_s=\AvgTime,MaxCPUTime_s=\MaxTime,AvgCost=\AvgCost,MaxCost=\MaxCost,InstanceCount=\Instances}{%
\Size & \AvgTime & \MaxTime & \AvgCost & \MaxCost}
\hline
\end{tabular}
\end{table}

\begin{figure}[ht]
\centering
\begin{tikzpicture}
\begin{axis}[
    xlabel={Tree size ($n$)},
    ylabel={Cost},
    width=\textwidth,
    height=7cm,
    grid=major,
    legend pos=north west,
    mark size=2pt
]
\addplot[blue, mark=*, thick] table[x=Size, y=AvgCost, col sep=comma] {data/dp_tree/dp_tree_basic_cb_analysis.csv};
\addplot[red, mark=square*, thick] table[x=Size, y=MaxCost, col sep=comma] {data/dp_tree/dp_tree_basic_cb_analysis.csv};
\legend{Avg Cost, Max Cost}
\end{axis}
\end{tikzpicture}
\caption{Random trees with binomial costs - DP exact algorithm: cost comparison}
\label{fig:random_c_binomial_dp_cost}
\end{figure}

\begin{figure}[ht]
\centering
\begin{tikzpicture}
\begin{axis}[
    xlabel={Tree size ($n$)},
    ylabel={Runtime (s)},
    width=\textwidth,
    height=7cm,
    grid=major,
    legend pos=north west,
    mark size=2pt
]
\addplot[blue, mark=*, thick] table[x=Size, y=AvgCPUTime_s, col sep=comma] {data/dp_tree/dp_tree_basic_cb_analysis.csv};
\addplot[red, mark=square*, thick] table[x=Size, y=MaxCPUTime_s, col sep=comma] {data/dp_tree/dp_tree_basic_cb_analysis.csv};
\legend{Avg Time, Max Time}
\end{axis}
\end{tikzpicture}
\caption{Random trees with binomial costs - DP exact algorithm: runtime comparison}
\label{fig:random_c_binomial_dp_time}
\end{figure}

\FloatBarrier

\subsubsection{Cicalese et al. $O(\log n / \log \log n)$-Approximation Algorithm}

\begin{table}[ht]
\centering
\tiny
\caption{Random trees with binomial cost distribution - Cicalese algorithm (every 2nd instance)}
\label{tab:random_c_binomial_cic}
\begin{tabular}{|c|c|c|c|c|c|c|}
\hline
$n$ & Avg Time (s) & Max Time (s) & Avg Cost & Max Cost & Avg Ratio & Max Ratio \\
\hline
\csvreader[head to column names, late after line=\\]{data/cic_insp/cic_insp_basic_cb_comparison.csv}{Size=\Size,AvgCPUTime_s=\AvgTime,MaxCPUTime_s=\MaxTime,AvgCost=\AvgCost,MaxCost=\MaxCost,AvgRatio=\AvgRatio,MaxRatio=\MaxRatio}{%
\Size & \AvgTime & \MaxTime & \AvgCost & \MaxCost & \AvgRatio & \MaxRatio}
\hline
\end{tabular}
\end{table}

\begin{figure}[ht]
\centering
\begin{tikzpicture}
\begin{axis}[
    xlabel={Tree size ($n$)},
    ylabel={Approximation ratio},
    width=\textwidth,
    height=7cm,
    grid=major,
    legend pos=north west,
    mark size=2pt
]
\addplot[blue, mark=*, thick] table[x=Size, y=AvgRatio, col sep=comma] {data/cic_insp/cic_insp_basic_cb_comparison.csv};
\addplot[red, mark=square*, thick] table[x=Size, y=MaxRatio, col sep=comma] {data/cic_insp/cic_insp_basic_cb_comparison.csv};
\legend{Avg Ratio, Max Ratio}
\end{axis}
\end{tikzpicture}
\caption{Random trees with binomial costs - Cicalese algorithm: approximation quality}
\label{fig:random_c_binomial_cic}
\end{figure}

\begin{figure}[ht]
\centering
\begin{tikzpicture}
\begin{axis}[
    xlabel={Tree size ($n$)},
    ylabel={Runtime (s)},
    width=\textwidth,
    height=7cm,
    grid=major,
    legend pos=north west,
    mark size=2pt
]
\addplot[blue, mark=*, thick] table[x=Size, y=AvgCPUTime_s, col sep=comma] {data/cic_insp/cic_insp_basic_cb_comparison.csv};
\addplot[red, mark=square*, thick] table[x=Size, y=MaxCPUTime_s, col sep=comma] {data/cic_insp/cic_insp_basic_cb_comparison.csv};
\legend{Avg Time, Max Time}
\end{axis}
\end{tikzpicture}
\caption{Random trees with binomial costs - Cicalese algorithm: runtime}
\label{fig:random_c_binomial_cic_time}
\end{figure}

\FloatBarrier

\subsubsection{Dereniowski $O(\sqrt{\log n})$-Approximation Algorithm}

\begin{table}[ht]
\centering
\tiny
\caption{Random trees with binomial cost distribution - Dereniowski algorithm (every 2nd instance)}
\label{tab:random_c_binomial_der}
\begin{tabular}{|c|c|c|c|c|c|c|}
\hline
$n$ & Avg Time (s) & Max Time (s) & Avg Cost & Max Cost & Avg Ratio & Max Ratio \\
\hline
\csvreader[head to column names, late after line=\\]{data/der_insp/der_insp_basic_cb_comparison.csv}{Size=\Size,AvgCPUTime_s=\AvgTime,MaxCPUTime_s=\MaxTime,AvgCost=\AvgCost,MaxCost=\MaxCost,AvgRatio=\AvgRatio,MaxRatio=\MaxRatio}{%
\Size & \AvgTime & \MaxTime & \AvgCost & \MaxCost & \AvgRatio & \MaxRatio}
\hline
\end{tabular}
\end{table}

\begin{figure}[ht]
\centering
\begin{tikzpicture}
\begin{axis}[
    xlabel={Tree size ($n$)},
    ylabel={Approximation ratio},
    width=\textwidth,
    height=7cm,
    grid=major,
    legend pos=north west,
    mark size=2pt
]
\addplot[blue, mark=*, thick] table[x=Size, y=AvgRatio, col sep=comma] {data/der_insp/der_insp_basic_cb_comparison.csv};
\addplot[red, mark=square*, thick] table[x=Size, y=MaxRatio, col sep=comma] {data/der_insp/der_insp_basic_cb_comparison.csv};
\legend{Avg Ratio, Max Ratio}
\end{axis}
\end{tikzpicture}
\caption{Random trees with binomial costs - Dereniowski algorithm: approximation quality}
\label{fig:random_c_binomial_der}
\end{figure}

\begin{figure}[ht]
\centering
\begin{tikzpicture}
\begin{axis}[
    xlabel={Tree size ($n$)},
    ylabel={Runtime (s)},
    width=\textwidth,
    height=7cm,
    grid=major,
    legend pos=north west,
    mark size=2pt
]
\addplot[blue, mark=*, thick] table[x=Size, y=AvgCPUTime_s, col sep=comma] {data/der_insp/der_insp_basic_cb_comparison.csv};
\addplot[red, mark=square*, thick] table[x=Size, y=MaxCPUTime_s, col sep=comma] {data/der_insp/der_insp_basic_cb_comparison.csv};
\legend{Avg Time, Max Time}
\end{axis}
\end{tikzpicture}
\caption{Random trees with binomial costs - Dereniowski algorithm: runtime}
\label{fig:random_c_binomial_der_time}
\end{figure}

\FloatBarrier

\subsubsection{K-up $O(\log \log n)$-Approximation Algorithm}

\begin{table}[ht]
\centering
\tiny
\caption{Random trees with binomial cost distribution - K-up algorithm (every 2nd instance)}
\label{tab:random_c_binomial_kup}
\begin{tabular}{|c|c|c|c|c|c|c|}
\hline
$n$ & Avg Time (s) & Max Time (s) & Avg Cost & Max Cost & Avg Ratio & Max Ratio \\
\hline
\csvreader[head to column names, late after line=\\]{data/k_up/k_up_basic_cb_comparison.csv}{Size=\Size,AvgCPUTime_s=\AvgTime,MaxCPUTime_s=\MaxTime,AvgCost=\AvgCost,MaxCost=\MaxCost,AvgRatio=\AvgRatio,MaxRatio=\MaxRatio}{%
\Size & \AvgTime & \MaxTime & \AvgCost & \MaxCost & \AvgRatio & \MaxRatio}
\hline
\end{tabular}
\end{table}

\begin{figure}[ht]
\centering
\begin{tikzpicture}
\begin{axis}[
    xlabel={Tree size ($n$)},
    ylabel={Approximation ratio},
    width=\textwidth,
    height=7cm,
    grid=major,
    legend pos=north west,
    mark size=2pt
]
\addplot[blue, mark=*, thick] table[x=Size, y=AvgRatio, col sep=comma] {data/k_up/k_up_basic_cb_comparison.csv};
\addplot[red, mark=square*, thick] table[x=Size, y=MaxRatio, col sep=comma] {data/k_up/k_up_basic_cb_comparison.csv};
\legend{Avg Ratio, Max Ratio}
\end{axis}
\end{tikzpicture}
\caption{Random trees with binomial costs - K-up algorithm: approximation quality}
\label{fig:random_c_binomial_kup}
\end{figure}

\begin{figure}[ht]
\centering
\begin{tikzpicture}
\begin{axis}[
    xlabel={Tree size ($n$)},
    ylabel={Runtime (s)},
    width=\textwidth,
    height=7cm,
    grid=major,
    legend pos=north west,
    mark size=2pt
]
\addplot[blue, mark=*, thick] table[x=Size, y=AvgCPUTime_s, col sep=comma] {data/k_up/k_up_basic_cb_comparison.csv};
\addplot[red, mark=square*, thick] table[x=Size, y=MaxCPUTime_s, col sep=comma] {data/k_up/k_up_basic_cb_comparison.csv};
\legend{Avg Time, Max Time}
\end{axis}
\end{tikzpicture}
\caption{Random trees with binomial costs - K-up algorithm: runtime}
\label{fig:random_c_binomial_kup_time}
\end{figure}

\FloatBarrier\subsection{Random Trees with Exponential Cost Distribution ($T_R || V, c \sim \mathcal{E} || \COST$)}
\label{sec:random_c_exponential}

\subsubsection{DP Exact Algorithm}

\begin{table}[ht]
\centering
\tiny
\caption{Random trees with exponential cost distribution - DP exact algorithm}
\label{tab:random_c_exponential_dp}
\begin{tabular}{|c|c|c|c|c|}
\hline
$n$ & Avg Time (s) & Max Time (s) & Avg Cost & Max Cost \\
\hline
\csvreader[head to column names, late after line=\\]{data/dp_tree/dp_tree_basic_ce_analysis.csv}{Size=\Size,AvgCPUTime_s=\AvgTime,MaxCPUTime_s=\MaxTime,AvgCost=\AvgCost,MaxCost=\MaxCost,InstanceCount=\Instances}{%
\Size & \AvgTime & \MaxTime & \AvgCost & \MaxCost}
\hline
\end{tabular}
\end{table}

\begin{figure}[ht]
\centering
\begin{tikzpicture}
\begin{axis}[
    xlabel={Tree size ($n$)},
    ylabel={Cost},
    width=\textwidth,
    height=7cm,
    grid=major,
    legend pos=north west,
    mark size=2pt
]
\addplot[blue, mark=*, thick] table[x=Size, y=AvgCost, col sep=comma] {data/dp_tree/dp_tree_basic_ce_analysis.csv};
\addplot[red, mark=square*, thick] table[x=Size, y=MaxCost, col sep=comma] {data/dp_tree/dp_tree_basic_ce_analysis.csv};
\legend{Avg Cost, Max Cost}
\end{axis}
\end{tikzpicture}
\caption{Random trees with exponential costs - DP exact algorithm: cost comparison}
\label{fig:random_c_exponential_dp_cost}
\end{figure}

\begin{figure}[ht]
\centering
\begin{tikzpicture}
\begin{axis}[
    xlabel={Tree size ($n$)},
    ylabel={Runtime (s)},
    width=\textwidth,
    height=7cm,
    grid=major,
    legend pos=north west,
    mark size=2pt
]
\addplot[blue, mark=*, thick] table[x=Size, y=AvgCPUTime_s, col sep=comma] {data/dp_tree/dp_tree_basic_ce_analysis.csv};
\addplot[red, mark=square*, thick] table[x=Size, y=MaxCPUTime_s, col sep=comma] {data/dp_tree/dp_tree_basic_ce_analysis.csv};
\legend{Avg Time, Max Time}
\end{axis}
\end{tikzpicture}
\caption{Random trees with exponential costs - DP exact algorithm: runtime comparison}
\label{fig:random_c_exponential_dp_time}
\end{figure}

\FloatBarrier

\subsubsection{Cicalese et al. $O(\log n / \log \log n)$-Approximation Algorithm}

\begin{table}[ht]
\centering
\tiny
\caption{Random trees with exponential cost distribution - Cicalese algorithm (every 2nd instance)}
\label{tab:random_c_exponential_cic}
\begin{tabular}{|c|c|c|c|c|c|c|}
\hline
$n$ & Avg Time (s) & Max Time (s) & Avg Cost & Max Cost & Avg Ratio & Max Ratio \\
\hline
\csvreader[head to column names, late after line=\\]{data/cic_insp/cic_insp_basic_ce_comparison.csv}{Size=\Size,AvgCPUTime_s=\AvgTime,MaxCPUTime_s=\MaxTime,AvgCost=\AvgCost,MaxCost=\MaxCost,AvgRatio=\AvgRatio,MaxRatio=\MaxRatio}{%
\Size & \AvgTime & \MaxTime & \AvgCost & \MaxCost & \AvgRatio & \MaxRatio}
\hline
\end{tabular}
\end{table}

\begin{figure}[ht]
\centering
\begin{tikzpicture}
\begin{axis}[
    xlabel={Tree size ($n$)},
    ylabel={Approximation ratio},
    width=\textwidth,
    height=7cm,
    grid=major,
    legend pos=north west,
    mark size=2pt
]
\addplot[blue, mark=*, thick] table[x=Size, y=AvgRatio, col sep=comma] {data/cic_insp/cic_insp_basic_ce_comparison.csv};
\addplot[red, mark=square*, thick] table[x=Size, y=MaxRatio, col sep=comma] {data/cic_insp/cic_insp_basic_ce_comparison.csv};
\legend{Avg Ratio, Max Ratio}
\end{axis}
\end{tikzpicture}
\caption{Random trees with exponential costs - Cicalese algorithm: approximation quality}
\label{fig:random_c_exponential_cic}
\end{figure}

\begin{figure}[ht]
\centering
\begin{tikzpicture}
\begin{axis}[
    xlabel={Tree size ($n$)},
    ylabel={Runtime (s)},
    width=\textwidth,
    height=7cm,
    grid=major,
    legend pos=north west,
    mark size=2pt
]
\addplot[blue, mark=*, thick] table[x=Size, y=AvgCPUTime_s, col sep=comma] {data/cic_insp/cic_insp_basic_ce_comparison.csv};
\addplot[red, mark=square*, thick] table[x=Size, y=MaxCPUTime_s, col sep=comma] {data/cic_insp/cic_insp_basic_ce_comparison.csv};
\legend{Avg Time, Max Time}
\end{axis}
\end{tikzpicture}
\caption{Random trees with exponential costs - Cicalese algorithm: runtime}
\label{fig:random_c_exponential_cic_time}
\end{figure}

\FloatBarrier

\subsubsection{Dereniowski $O(\sqrt{\log n})$-Approximation Algorithm}

\begin{table}[ht]
\centering
\tiny
\caption{Random trees with exponential cost distribution - Dereniowski algorithm (every 2nd instance)}
\label{tab:random_c_exponential_der}
\begin{tabular}{|c|c|c|c|c|c|c|}
\hline
$n$ & Avg Time (s) & Max Time (s) & Avg Cost & Max Cost & Avg Ratio & Max Ratio \\
\hline
\csvreader[head to column names, late after line=\\]{data/der_insp/der_insp_basic_ce_comparison.csv}{Size=\Size,AvgCPUTime_s=\AvgTime,MaxCPUTime_s=\MaxTime,AvgCost=\AvgCost,MaxCost=\MaxCost,AvgRatio=\AvgRatio,MaxRatio=\MaxRatio}{%
\Size & \AvgTime & \MaxTime & \AvgCost & \MaxCost & \AvgRatio & \MaxRatio}
\hline
\end{tabular}
\end{table}

\begin{figure}[ht]
\centering
\begin{tikzpicture}
\begin{axis}[
    xlabel={Tree size ($n$)},
    ylabel={Approximation ratio},
    width=\textwidth,
    height=7cm,
    grid=major,
    legend pos=north west,
    mark size=2pt
]
\addplot[blue, mark=*, thick] table[x=Size, y=AvgRatio, col sep=comma] {data/der_insp/der_insp_basic_ce_comparison.csv};
\addplot[red, mark=square*, thick] table[x=Size, y=MaxRatio, col sep=comma] {data/der_insp/der_insp_basic_ce_comparison.csv};
\legend{Avg Ratio, Max Ratio}
\end{axis}
\end{tikzpicture}
\caption{Random trees with exponential costs - Dereniowski algorithm: approximation quality}
\label{fig:random_c_exponential_der}
\end{figure}

\begin{figure}[ht]
\centering
\begin{tikzpicture}
\begin{axis}[
    xlabel={Tree size ($n$)},
    ylabel={Runtime (s)},
    width=\textwidth,
    height=7cm,
    grid=major,
    legend pos=north west,
    mark size=2pt
]
\addplot[blue, mark=*, thick] table[x=Size, y=AvgCPUTime_s, col sep=comma] {data/der_insp/der_insp_basic_ce_comparison.csv};
\addplot[red, mark=square*, thick] table[x=Size, y=MaxCPUTime_s, col sep=comma] {data/der_insp/der_insp_basic_ce_comparison.csv};
\legend{Avg Time, Max Time}
\end{axis}
\end{tikzpicture}
\caption{Random trees with exponential costs - Dereniowski algorithm: runtime}
\label{fig:random_c_exponential_der_time}
\end{figure}

\FloatBarrier

\subsubsection{K-up $O(\log \log n)$-Approximation Algorithm}

\begin{table}[ht]
\centering
\tiny
\caption{Random trees with exponential cost distribution - K-up algorithm (every 2nd instance)}
\label{tab:random_c_exponential_kup}
\begin{tabular}{|c|c|c|c|c|c|c|}
\hline
$n$ & Avg Time (s) & Max Time (s) & Avg Cost & Max Cost & Avg Ratio & Max Ratio \\
\hline
\csvreader[head to column names, late after line=\\]{data/k_up/k_up_basic_ce_comparison.csv}{Size=\Size,AvgCPUTime_s=\AvgTime,MaxCPUTime_s=\MaxTime,AvgCost=\AvgCost,MaxCost=\MaxCost,AvgRatio=\AvgRatio,MaxRatio=\MaxRatio}{%
\Size & \AvgTime & \MaxTime & \AvgCost & \MaxCost & \AvgRatio & \MaxRatio}
\hline
\end{tabular}
\end{table}

\begin{figure}[ht]
\centering
\begin{tikzpicture}
\begin{axis}[
    xlabel={Tree size ($n$)},
    ylabel={Approximation ratio},
    width=\textwidth,
    height=7cm,
    grid=major,
    legend pos=north west,
    mark size=2pt
]
\addplot[blue, mark=*, thick] table[x=Size, y=AvgRatio, col sep=comma] {data/k_up/k_up_basic_ce_comparison.csv};
\addplot[red, mark=square*, thick] table[x=Size, y=MaxRatio, col sep=comma] {data/k_up/k_up_basic_ce_comparison.csv};
\legend{Avg Ratio, Max Ratio}
\end{axis}
\end{tikzpicture}
\caption{Random trees with exponential costs - K-up algorithm: approximation quality}
\label{fig:random_c_exponential_kup}
\end{figure}

\begin{figure}[ht]
\centering
\begin{tikzpicture}
\begin{axis}[
    xlabel={Tree size ($n$)},
    ylabel={Runtime (s)},
    width=\textwidth,
    height=7cm,
    grid=major,
    legend pos=north west,
    mark size=2pt
]
\addplot[blue, mark=*, thick] table[x=Size, y=AvgCPUTime_s, col sep=comma] {data/k_up/k_up_basic_ce_comparison.csv};
\addplot[red, mark=square*, thick] table[x=Size, y=MaxCPUTime_s, col sep=comma] {data/k_up/k_up_basic_ce_comparison.csv};
\legend{Avg Time, Max Time}
\end{axis}
\end{tikzpicture}
\caption{Random trees with exponential costs - K-up algorithm: runtime}
\label{fig:random_c_exponential_kup_time}
\end{figure}
