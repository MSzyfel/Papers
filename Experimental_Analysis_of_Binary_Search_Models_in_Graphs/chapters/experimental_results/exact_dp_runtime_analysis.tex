...
During the execution of the exact dynamic programming algorithm from Section~\ref{sec:log_n_log_log_n_approx}, it could be noticed that often, the running time of the procedure was slightly better then $O\br{2^nn}$. We decided to find the value of $b$ such that the function $T(n) = b^n$ best approximates the running time of the procedure. 
...
\begin{itemize}
    ...
    \item Random trees with bounded degree (Degree 3) exhibit strong exponential growth pattern ($R^2 > 0.91$ for average time), with bases $b$ ranging from 1.353 to 1.379.
    \item Random trees with bounded diameter (Diameter 6) display the highest growth bases ($b \approx 1.5$-$1.6$), consistent with their more balanced structure requiring exploration of more subtrees.
\end{itemize}
